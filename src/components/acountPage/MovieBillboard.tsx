/* eslint-disable @typescript-eslint/no-explicit-any */
"use client";

import React, { useEffect, useState } from "react";
import { Info } from "lucide-react";
import HomePageHeader from "@/src/ui/header/HomePageHeader";
import { useTranslations } from "next-intl";
import PlayButton from "@/src/ui/PlayButton";

interface BillboardProps {
  onOpenModal: (movie: { id: string; title: string; thumbnailUrl: string; type?: "movie" | "tv" }) => void; 
}

const MovieBillboard: React.FC<BillboardProps> = ({ onOpenModal }) => {
  const t = useTranslations("Billboard");
  const [movie, setMovie] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  const MOVIE_ID = "1419406"; 
  const API_KEY = process.env.NEXT_PUBLIC_TMDB_API_KEY;

  useEffect(() => {
    const fetchMovie = async () => {
      try {
        setLoading(true);
        // فراخوانی دیتای فیلم از TMDB
        const response = await fetch(
          `https://api.themoviedb.org/3/movie/${MOVIE_ID}?api_key=${API_KEY}&language=en-US`
        );
        
        if (!response.ok) throw new Error("Movie not found or network error");
        
        const data = await response.json();
        setMovie(data);
      } catch (error) {
        console.error("Error retrieving movie information:", error);
      } finally {
        setLoading(false);
      }
    };

    if (API_KEY) {
      fetchMovie();
    }
  }, [API_KEY]);

  if (loading || !movie) {
    return <div className="h-[45vh] md:h-[56.25vw] w-full bg-zinc-950 animate-pulse" />;
  }

  const backdropUrl = `https://image.tmdb.org/t/p/original${movie.backdrop_path}`;

  return (
    <div
      className="relative h-[45vh] md:h-[56.25vw] w-full bg-no-repeat bg-cover bg-center transition-all duration-500 bg-zinc-950"
      style={{ backgroundImage: `url('${backdropUrl}')` }}
    >
      <div className="absolute inset-0 bg-linear-to-r from-black/80 via-transparent to-transparent z-10" />
      <div className="absolute inset-0 bg-linear-to-t from-black via-black/20 to-transparent z-10" />
      
      <HomePageHeader />

      <div className="absolute bottom-[15%] md:bottom-[30%] left-6 md:left-20 z-20 max-w-xs md:max-w-xl flex flex-col gap-2 md:gap-4">
        <h1 className="text-white text-2xl md:text-5xl lg:text-7xl font-black drop-shadow-xl tracking-tighter uppercase italic">
          {movie.title}
        </h1>

        <p className="text-gray-300 text-[10px] md:text-sm lg:text-base line-clamp-3 md:line-clamp-4 drop-shadow-md font-light max-w-md leading-relaxed">
          {movie.overview}
        </p>

        <div className="flex items-center gap-3 mt-2 md:mt-4">
          <PlayButton movieId={MOVIE_ID} mediaType="movie" />

          <button
            onClick={() => {
              onOpenModal({
                id: MOVIE_ID,
                title: movie.title,
                thumbnailUrl: backdropUrl,
                type: "movie",
              });
            }}
            className="bg-gray-500/30 text-white rounded-sm md:rounded-md py-1.5 md:py-2 px-4 md:px-6 flex items-center gap-2 text-xs md:text-base font-semibold hover:bg-gray-500/50 transition-colors backdrop-blur-sm"
          >
            <Info size={18} />
            <span>{t("moreInfo")}</span>
          </button>
        </div>
      </div>
    </div>
  );
};

export default MovieBillboard;