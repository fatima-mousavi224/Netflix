...
const BrowseMoviesPage = () => {
  ...
  const getMovieTag = (movie: Movie, index: number) => {
    ...
    if (movie.vote_average > 7.5) return t("topRated");
    return null;
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-800/60 pb-6 relative z-50">
          ...
          <div className="flex flex-wrap items-center gap-6 text-xs md:text-sm">
            <div className="flex flex-col gap-1 relative">
              ...
              {isLangOpen && (
                <div className="absolute top-full left-0 mt-2 w-48 bg-zinc-950 border border-zinc-800 rounded shadow-2xl overflow-hidden py-1 max-h-60 overflow-y-auto no-scrollbar z-50">
                  {LANGUAGES.map((lang) => (
                    <div
                      key={lang.code}
                      onClick={() => {
                      ...
                      className="px-4 py-2 hover:bg-zinc-900 cursor-pointer flex items-center justify-between text-zinc-300 hover:text-white transition-colors"
                    >
                      <span>{lang.name}</span>
            ...
            <div className="flex flex-col gap-1 relative ml-auto md:ml-4">
              ...
              {isSortOpen && (
                <div className="absolute top-full right-0 mt-2 w-64 bg-zinc-950 border border-zinc-800 rounded shadow-2xl overflow-hidden py-1 z-50">
              ...
              return (
                ...
                >
                  <div className="relative aspect-video w-full bg-zinc-900 rounded-sm overflow-hidden shadow-md group-hover:shadow-2xl">
                    ...
                    <div className="absolute top-2 left-2 tracking-tighter z-20">
                      <Image
                    ...
                    {tag && (
                      <div className="absolute bottom-0 left-0 right-0 flex justify-center z-20">
                        <span className="text-[10px] md:text-[11px] rounded-xs font-semibold uppercase px-3 py-1 tracking-wider text-center text-white shadow-md bg-red-600 block">
                          {tag}
                        </span>
                      </div>
                    )}
